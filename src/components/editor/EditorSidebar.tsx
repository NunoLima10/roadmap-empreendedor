...
export const EditorSidebar = ({ roadmap, onRoadmapChange, onAddNode, onSelectNode }: EditorSidebarProps) => {
  ...
  const handleChange = (field: keyof Roadmap, value: any) => {
    onRoadmapChange({ ...roadmap, [field]: value });
  };

  return (
    <div className="w-[300px] flex flex-col bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 h-full overflow-hidden">
      <div className="p-4 border-b border-slate-200 dark:border-slate-800">
        <h2 className="font-bold text-lg">Roadmap Editor</h2>
        <p className="text-xs text-slate-500">Client-side only. Export to save.</p>
      </div>

      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {/* Global Settings */}
        <div className="space-y-3">
          <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Global Settings</h3>
        ...
        <div className="space-y-3">
           <div className="flex items-center justify-between">
              <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Nodes ({roadmap.nodes.length})</h3>
              <button 
                onClick={onAddNode}
           ...
           <div className="space-y-1">
             {roadmap.nodes.map(node => (
               ...
               >
                 <span className={`w-2 h-2 rounded-full ${node.status === 'completed' ? 'bg-green-400' : 'bg-slate-300'}`} />
                 <span className="truncate flex-1">{node.title}</span>
                 <span className="text-xs text-slate-400 opacity-0 group-hover:opacity-100">{node.id}</span>
               </div>
             ))}
           </div>
        </div>
