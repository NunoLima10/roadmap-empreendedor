...
export const NodeEditor = ({
  ...
  const handleChange = (
    field: keyof RoadmapNode,
    value: string | string[] | NodeStatus | undefined,
  ) => {
  ...
  return (
    <>
      ...
      {/* Editor Panel */}
      <div className="fixed lg:relative inset-0 lg:inset-auto z-50 lg:z-auto h-full flex flex-col bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 shadow-xl lg:w-[400px] w-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              onChange={(e) =>
                handleChange("status", e.target.value as NodeStatus)
              }
              className="w-full px-3 py-2 bg-white dark:bg-slate-950 rounded-md border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-blue-500 outline-none text-sm"
            >
            ...
            <div className="border border-slate-200 dark:border-slate-700 rounded-md max-h-40 overflow-y-auto bg-slate-50 dark:bg-slate-950/50 p-2 space-y-1">
              {allNodes
                ...
                .map((n) => (
                  <label
                    key={n.id}
                    className="flex items-center gap-2 text-sm p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded cursor-pointer"
                  >
                    <input
                      type="checkbox"
                      checked={(node.dependsOn || []).includes(n.id)}
                      onChange={(e) =>
                        handleDependsOnChange(n.id, e.target.checked)
                      }
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              className="w-full px-3 py-2 bg-white dark:bg-slate-950 rounded-md border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-blue-500 outline-none font-mono text-xs h-64"
            />
          </div>
